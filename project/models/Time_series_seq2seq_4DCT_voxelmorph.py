...
from .unet_utils import ConvOut


# 3D CNN
class Encoder3DCNN(nn.Module):
    """
    3D CNN encoder for 2D images in time series.
    """
    ...
    def __init__(self, in_channels, out_channels):
        ...
        )  # 原本老陈写的是这个，2d正方形图片整个输入用这个
        # self.pool1 = nn.MaxPool3d(kernel_size=(1, 2, 1), stride=(1, 2, 1)) # 当输入2d图片改变为左右两半，就需要用这个，不然shape不对
        ...
        self.conv4 = nn.Conv3d(
            256, out_channels, kernel_size=1
        )  # Reduce channel depth to desired output
...
class EncoderDecoderConvLSTM(nn.Module):
    def __init__(self, nf, in_chan, size1, size2, size3):
        ...
        self.ConvLSTM3d1 = ConvLSTMCell(
            input_dim=nf, hidden_dim=nf, kernel_size=(3, 3, 3), bias=True
        )
        self.ConvLSTM3d2 = ConvLSTMCell(
            input_dim=nf, hidden_dim=nf, kernel_size=(3, 3, 3), bias=True
        )
        self.ConvLSTM3d3 = ConvLSTMCell(
            input_dim=nf, hidden_dim=nf, kernel_size=(3, 3, 3), bias=True
        )
        self.ConvLSTM3d4 = ConvLSTMCell(
            input_dim=nf, hidden_dim=nf, kernel_size=(3, 3, 3), bias=True
        )
    ...
    def autoencoder(
        ...
        for t in range(seq_len - 1, -1, -1):  # In3 Out3 use This
            # 应用3D CNN encoder
            time_series_fat_encoder = self.encoder3d_cnn(
                batch_2D[:, :, :-1, ...]
            )  # batch_2D torch.Size([1, 3, 3, 128, 128])
            # time_series_fat = self.encoder3d_cnn(batch_2D[:, :, :, ...])

            h_t1 = self.encoder1_conv(x[:, t, ...])
            down1 = self.down1(h_t1)

            h_t4, c_t4 = self.ConvLSTM3d1(input_tensor=down1, cur_state=[h_t4, c_t4])
            h_t5, c_t5 = self.ConvLSTM3d2(
                input_tensor=h_t4,  # c_t5 h_t5.shape=>[1,96,35,60,70]  input:(nf=96, in_chan=1, size1=70, size2=120, size3=140)
                cur_state=[h_t5, c_t5],
            )

            # check shape
            assert len(h_t5.shape) == len(
                time_series_fat_encoder.shape
            ), "the dimension of h_t5 and batch_2D_encoded is not same."

            # fuse the 4DCT feature and time series feature, for encoder
            # encoder_vector = h_t5 @ time_series_fat # 相乘操作
        ...
        for t in range(future_step):  # 这里不用改成future_step-1
            ...
            h_t6, c_t6 = self.ConvLSTM3d3(
                input_tensor=encoder_vector, cur_state=[h_t6, c_t6]
            )
            h_t7, c_t7 = self.ConvLSTM3d4(
                input_tensor=h_t6,  # c_t7 h_t7.shape=>[1,96,35,60,70]  input:(nf=96, in_chan=1, size1=70, size2=120, size3=140)
                cur_state=[h_t7, c_t7],
            )

            # check shape
            assert len(h_t7.shape) == len(
                time_series_fat_decoder.shape
            ), "the dimension of h_t7 and batch_2D_encoded is not same."

            # fuse the 4DCT feature and time series feature, for decoder
            # decoder_vector = h_t7 @ time_series_fat # 相乘操作
            decoder_vector = h_t7 + time_series_fat_decoder  # 相加操作
        ...
        for i in range(timestep):
            output_ts = self.up1(
                latent[:, :, i, ...]
            )  # output_ts torch.Size([1, 96, 70, 120, 140])
            ...
            warped_img = self.transformer(
                x[:, 0, ...], dvf
            )  # warped_img torch.Size([1, 1, 70, 120, 140]), x torch.Size([1, 4, 1, 70, 120, 140])
            output_img += [warped_img]  #
            output_dvf += [dvf]

        output_img = torch.stack(
            output_img, 1
        )  # output_img torch.Size([1, 3, 1, 70, 120, 140])
        output_dvf = torch.stack(
            output_dvf, 1
        )  # output_dvf torch.Size([1, 3, 3, 70, 120, 140])
        output_img = output_img.permute(
            0, 2, 1, 3, 4, 5
        )  # output_img torch.Size([1, 1, 3, 70, 120, 140])
        output_dvf = output_dvf.permute(
            0, 2, 1, 3, 4, 5
        )  # output_dvf torch.Size([1, 3, 3, 70, 120, 140])
    ...
    def forward(self, x, batch_2D, future_seq=0, hidden_state=None):
        ...
        h_t4, c_t4 = self.ConvLSTM3d1.init_hidden(
            batch_size=b, image_size=(int(d // 2), int(h // 2), int(w // 2))
        )
        h_t5, c_t5 = self.ConvLSTM3d2.init_hidden(
            batch_size=b, image_size=(int(d // 2), int(h // 2), int(w // 2))
        )
        h_t6, c_t6 = self.ConvLSTM3d3.init_hidden(
            batch_size=b, image_size=(int(d // 2), int(h // 2), int(w // 2))
        )
        h_t7, c_t7 = self.ConvLSTM3d4.init_hidden(
            batch_size=b, image_size=(int(d // 2), int(h // 2), int(w // 2))
        )  #!origin
